...
class ScholarSkills extends Component {

  constructor(props) {
    super(props);

    this.state = {
      ScholarSkillData: undefined
    }
  }
  ...
  render() {
    ...
    // Handle case where the response is not here yet
      if ( !this.state.ScholarSkillData ) {
      ...
      if ( this.state.ScholarSkillData.length === 0 ) {
        return (
          <ScrollView style={styles.defaultContainerWithPadding}>
      ...
      return (
        <ScrollView style={styles.defaultContainer}>
          <List style={styles.defaultContainer}>
            {this.state.ScholarSkillData.map((skill) => (
              <ListItem
                key={skill.name}
